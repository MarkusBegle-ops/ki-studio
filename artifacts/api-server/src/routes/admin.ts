...
router.get("/download/app", async (_req: Request, res: Response) => {
  try {
    const filePath = path.join(REPO_ROOT, "artifacts/ki-studio/public/KI-Studio-App.txt");
    const content = await fs.readFile(filePath, "utf-8");
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
    res.setHeader("Content-Disposition", 'attachment; filename="KI-Studio-App.txt"');
    res.setHeader("Cache-Control", "no-store");
    res.send(content);
  } catch {
    res.status(404).json({ error: "Datei nicht gefunden" });
